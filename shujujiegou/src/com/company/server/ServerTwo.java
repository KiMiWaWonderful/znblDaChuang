...
public class ServerTwo {
     ...
     */
    public ServerTwo(int Port, int IDMode)
    {
        idmode = IDMode;
        // int idmode = IDMode;
    ...
    {
        // 获取迭代器
        Iterator<Map.Entry<String, Socket>> it =
                clientList.entrySet().iterator();

        while(it.hasNext())
        {
            // 取出下一个迭代器的内容
            Map.Entry<String, Socket> nvp = it.next();
            // 判断是否是数据源，是就跳过
                ...
                while(true)
                {
                    ...
                    if(buff[0] == '<')
                    {
                        ...
                        if(gt_loc > 0 && gt_loc < len)
                        {
                            ...
                            for (int i = 0; i <dest_list.length ; i++) {
                                System.out.print(dest_list[i] + " ");
                            }
                            System.out.println();
                            // 复制剩余部分
    ...
    public static void main(String[] args)
    {
      ...
      //  int port2 = 33334;
        ...
        // 创建对象
        ServerTwo server = new ServerTwo(port, mode);
        //Server33333 server2 = new Server33333(port, mode);
        // 启动等待
        server.start();
       // server2.start();
    }
